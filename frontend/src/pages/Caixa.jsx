...
/** Largura útil da comanda impressa (bobina 80 mm) */
const COMANDA_IMPRESSAO_LARGURA_MM = 80
...
function buildComandaPrintHtml(d) {
...
export default function Caixa() {
  ...
  const printComanda = () => {
    ...
    getPrintComanda(summary.comanda.id).then((d) => {
      const html = buildComandaPrintHtml(d)
      const win = window.open('', '_blank')
      if (!win) return
      win.document.open()
      win.document.write(html)
      win.document.close()
      win.document.title = `Comanda ${d.comanda}`
      const runPrint = () => {
        try {
          win.focus()
          win.print()
