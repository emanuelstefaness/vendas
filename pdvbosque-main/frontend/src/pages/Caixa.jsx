...
export default function Caixa() {
  ...
  useEffect(() => {
    ...
    try {
      if (typeof navigator !== 'undefined' && navigator.vibrate) navigator.vibrate([280, 120, 280])
    } catch (_) {}
  ...
  return (
    <div className="min-h-0 space-y-6 pb-8">
      {onlineOrderAlert && (
        <>
          {/* Anel vermelho + faixas fixas: “spam” visual acima de tudo (nav z-50) */}
          <div
            className="pointer-events-none fixed inset-0 z-[9998] animate-pulse shadow-[inset_0_0_0_10px_rgba(220,38,38,0.95),inset_0_0_60px_rgba(250,204,21,0.2)]"
            aria-hidden
          />
          <div className="pointer-events-none fixed inset-x-0 top-0 z-[9999] bg-red-600 py-2.5 text-center text-[11px] font-black uppercase leading-tight tracking-widest text-white shadow-lg sm:text-sm animate-pulse">
            🆕 NOVO PEDIDO ONLINE — #{onlineOrderAlert.orderId ?? '—'} — confirme no centro da tela
          </div>
          <div className="pointer-events-none fixed inset-x-0 bottom-0 z-[9999] bg-red-600 py-2.5 text-center text-[11px] font-black uppercase leading-tight tracking-widest text-white shadow-lg sm:text-sm animate-pulse">
            NOVO PEDIDO #{onlineOrderAlert.orderId ?? '—'} — {onlineOrderAlert.cliente_nome || 'Cliente'}
          </div>
          <div
          ...
          >
            <div className="pointer-events-auto w-full max-w-lg rounded-2xl border-4 border-red-600 bg-white p-6 shadow-2xl ring-4 ring-amber-400/90 sm:p-8">
              <p className="text-center text-xs font-black uppercase tracking-[0.25em] text-red-600">Pedido online recebido</p>
              <h2 id="caixa-novo-pedido-titulo" className="mt-2 text-center text-3xl font-black text-slate-900 sm:text-4xl">
              ...
              <p className="mt-4 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-center text-xs font-semibold text-amber-950">
                O som toca em loop até você responder. <strong>Sim</strong> abre Pedidos online; <strong>Não</strong> só fecha este aviso no caixa.
              </p>
